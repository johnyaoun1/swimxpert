...
var (dataProtectionCert, dataProtectionError) = LoadDataProtectionCertificate();
var dataProtection = builder.Services.AddDataProtection()
    .SetApplicationName("SwimXpert")
    .PersistKeysToDbContext<ApplicationDbContext>();
if (dataProtectionCert is not null)
    dataProtection.ProtectKeysWithCertificate(dataProtectionCert);
builder.Services.AddSingleton(new CalendarEncryptionStatus
{
    Ready = dataProtectionCert is not null,
    LoadError = dataProtectionError
});
builder.Services.AddSingleton<GoogleRefreshTokenProtector>();
...
var calendarEncryption = app.Services.GetRequiredService<CalendarEncryptionStatus>();
if (!calendarEncryption.Ready)
    app.Logger.LogError("Google Calendar sync disabled: {Reason}", calendarEncryption.LoadError);
...
using (var scope = app.Services.CreateScope())
{
    ...
    // Encrypt only after the certificate loaded. Without it, Protect would store an
    // unwrapped key next to the token. Leave any existing plaintext untouched until then.
    var encryption = scope.ServiceProvider.GetRequiredService<CalendarEncryptionStatus>();
    if (encryption.Ready)
    {
        var tokenProtector = scope.ServiceProvider.GetRequiredService<GoogleRefreshTokenProtector>();
        var calendarState = await db.GoogleCalendarStates.FirstOrDefaultAsync(s => s.Id == 1);
        if (calendarState?.RefreshToken is { Length: > 0 } storedToken && !GoogleRefreshTokenProtector.IsProtected(storedToken))
        {
            calendarState.RefreshToken = tokenProtector.Protect(storedToken);
            calendarState.UpdatedAt = DateTime.UtcNow;
            await db.SaveChangesAsync();
            app.Logger.LogInformation("Encrypted the stored Google Calendar refresh token. The plaintext value was replaced.");
        }
    }
}
...
static (X509Certificate2? Cert, string? Error) LoadDataProtectionCertificate()
{
    var encoded = Environment.GetEnvironmentVariable("DATA_PROTECTION_CERT");
    var password = Environment.GetEnvironmentVariable("DATA_PROTECTION_CERT_PASSWORD");
    var hasCert = !string.IsNullOrWhiteSpace(encoded);
    var hasPassword = !string.IsNullOrEmpty(password);
    if (!hasCert && !hasPassword)
        return (null, "DATA_PROTECTION_CERT and DATA_PROTECTION_CERT_PASSWORD are not set.");

    if (!hasCert || !hasPassword)
        return (null, "Set both DATA_PROTECTION_CERT and DATA_PROTECTION_CERT_PASSWORD.");

    byte[] raw;
    try
    {
        var compact = string.Concat(encoded!.Where(static c => !char.IsWhiteSpace(c)));
        raw = Convert.FromBase64String(compact);
    }
    catch (FormatException)
    {
        return (null, "DATA_PROTECTION_CERT is not valid base64.");
    }

    X509Certificate2 cert;
    try
    {
        cert = X509CertificateLoader.LoadPkcs12(raw, password, X509KeyStorageFlags.EphemeralKeySet);
    }
    catch (CryptographicException)
    {
        return (null, "DATA_PROTECTION_CERT could not be loaded. Check the PFX and DATA_PROTECTION_CERT_PASSWORD.");
    }

    if (!cert.HasPrivateKey)
    {
        cert.Dispose();
        return (null, "DATA_PROTECTION_CERT must include the private key.");
    }
